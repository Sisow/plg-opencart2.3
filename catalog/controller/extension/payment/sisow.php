...
class ControllerExtensionPaymentSisow extends Controller {
	...
	public function _index($payment) {
		...
		if($this->config->get($payment . '_useb2b') == 'true')
			$payment .= 'b2b';
		...
		{		
			...
			*/
			...
			foreach ($products_info as $key => $data) {
				...
				// product total
				//$unit_price = $this->tax->calculate($data['price'], $data['tax_class_id'], $this->config->get('config_tax'));
			...
			foreach ($results as $result) {
				if ($this->config->get($result['code'] . '_status')) {
					...
					if ($numberOfTotals != count($total_data['totals']) && (end($total_data['totals'])['code'] != 'sub_total' && end($total_data['totals'])['code'] != 'tax' && end($total_data['totals'])['code'] != 'total')) {
						...
						$product_id = end($total_data['totals'])['code'] == 'sisowfeetotal' ? 'paymentfee' : end($total_data['totals'])['code'];
	...
	public function _notify($payment) {
		...
		if (isset($this->request->get['trxid'])) {
			...
			if(!empty($trxid) && $order_info['order_status_id'] != $this->config->get($payment . '_status_success'))
			{
				...
				if (($ex = $sisow->StatusRequest($trxid)) < 0) {
					...
					{
						$this->model_checkout_order->update($order_id, 7, 'Overboeking Cancelled!', false);
					}
				}
			}
		}
