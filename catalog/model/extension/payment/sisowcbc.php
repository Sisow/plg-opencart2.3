...
class ModelExtensionPaymentSisowCbc extends Model {
	public function getMethod($address = false, $total = false) {
		...
		$this->load->language('extension/payment/sisowcbc');
		$data = array( 
			'terms'      => '',
			'code'		=> 'sisowcbc',
			'title'		=> $this->language->get('text_title').($this->config->get('sisowcbc_testmode') == 'true' ? ' [Testmode]' : ''),
