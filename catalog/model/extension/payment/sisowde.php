...
class ModelExtensionPaymentSisowDE extends Model {
	public function getMethod($address = false, $total = false) {
		...
		$this->load->language('extension/payment/sisowde');
		$data = array( 
			'terms'      => '',
			'code'		=> 'sisowde',
			'title'		=> $this->language->get('text_title').($this->config->get('sisowde_testmode') == 'true' ? ' [Testmode]' : ''),
