...
class ModelExtensionPaymentSisowKbc extends Model {
	public function getMethod($address = false, $total = false) {
		...
		$this->load->language('extension/payment/sisowkbc');
		$data = array( 
			'terms'      => '',
			'code'		=> 'sisowkbc',
			'title'		=> $this->language->get('text_title').($this->config->get('sisowkbc_testmode') == 'true' ? ' [Testmode]' : ''),
