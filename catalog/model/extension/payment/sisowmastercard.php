...
class ModelExtensionPaymentsisowMastercard extends Model {
	public function getMethod($address = false, $total = false) {
		...
		$this->load->language('extension/payment/sisowmastercard');
		$data = array(
			'terms'      => '',
			'code'		=> 'sisowmastercard',
			'title'		=> $this->language->get('text_title').($this->config->get('sisowmastercard_testmode') == 'true' ? ' [Testmode]' : ''),
