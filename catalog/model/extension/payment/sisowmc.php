...
class ModelExtensionPaymentSisowMC extends Model {
	public function getMethod($address = false, $total = false) {
		...
		$this->load->language('extension/payment/sisowmc');
		$data = array(
			'terms'      => '',
			'code'		=> 'sisowmc',
			'title'		=> $this->language->get('text_title').($this->config->get('sisowmc_testmode') == 'true' ? ' [Testmode]' : ''),
