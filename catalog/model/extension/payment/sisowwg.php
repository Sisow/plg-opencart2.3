...
class ModelExtensionPaymentSisowWG extends Model {
	public function getMethod($address = false, $total = false) {
		...
		$this->load->language('extension/payment/sisowwg');
		$data = array(
			'terms'      => '',
			'code'		=> 'sisowwg',
			'title'		=> $this->language->get('text_title').($this->config->get('sisowwg_testmode') == 'true' ? ' [Testmode]' : ''),
